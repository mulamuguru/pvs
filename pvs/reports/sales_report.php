...
// Fetch loans
$loanStmt = $conn->prepare("SELECT principal_amount, created_at FROM loans WHERE status = 'active' AND officer_id = ? AND DATE(created_at) BETWEEN ? AND ?");
$loanStmt->execute([$officerId, $startDate, $endDate]);
...
$orderStmt->execute([$officerId, $startDate, $endDate]);
